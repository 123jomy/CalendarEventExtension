...
function validateresourcesnames {
    ...
    $authorizationtoken = get-accesstokenfromcurrentuser -erroraction Inquire
    $resources = @(@{
    ...
    foreach ($resource in $resources) {
        $isresourcenameavailable = validateresourcenames $resource -erroraction Inquire
...
function validateresourcenames {
    ...
    if ($resourceinfo.servicetype -eq "applicationinsights") {
        ...
        $availabilityresult = IsResourceNameAvailable @resourceinfo -erroraction Inquire
...
function logout {
    ...
    $exit = Read-Host -Prompt 'Logged Out. Press any key to exit..'
}
...
# Load Parameters from JSON meta-data file
    $parametersListContent = Get-Content '.\parameters.json' -ErrorAction Inquire
...
# Initialize connections - Azure Az/CLI/Azure AD
    ...
    Connect-AzAccount -Subscription $parameters.subscriptionId.Value -ErrorAction Inquire
    ...
    $ADaccount = Connect-AzureAD -Tenant $parameters.subscriptionTenantId.Value -ErrorAction Inquire
